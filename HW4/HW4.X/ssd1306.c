...
#include "ssd1306.h"
#include "font.h" // for ascii table
...
// define width for each character
const unsigned short CHAR_WIDTH = 5;
...
// Draw character
void ssd1306_drawCharacter(unsigned char x, unsigned char y, unsigned char c) {
    // i = col, j = row
    unsigned int i, j;
...
void ssd1306_drawMessage(unsigned char x, unsigned char y, char* msg) {
    ...
    for (i=0; i<strlen(msg); i++) {
        ssd1306_drawCharacter(x + i*CHAR_WIDTH, y, msg[i]);
    }
}