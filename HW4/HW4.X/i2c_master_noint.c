...
void i2c_master_write(unsigned char device_address, unsigned char device_register, unsigned char data){
    // start
    i2c_master_start();
    // send device address
    i2c_master_send((device_address << 1) | 0x0); // 7 bit address + 0 (write)
    // send device register
    i2c_master_send(device_register);
    // send data to set on register
    i2c_master_send(data);
    // stop
    i2c_master_stop();
}

unsigned char i2c_master_read(unsigned char device_address, unsigned char device_register){
    unsigned char data;
    // start
    i2c_master_start();
    // send device address
    i2c_master_send((device_address << 1) | 0x0); // 7 bit addrsss + 0 (write)
    // send device register (to read)
    i2c_master_send(device_register);
    // restart bit
    i2c_master_restart();
    // address to read
    i2c_master_send((device_address << 1) | 0x1); // 7 bit address + 1 (read)
    // receive data
